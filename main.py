...
class EarthquakeDashboard(tk.Tk):
    ...
    def _apply_ttk_theme(self):
        ...
        style.configure('.',
            ...
            troughcolor=BG_MID, selectbackground=ACCENT,
        ...
        style.configure('TCombobox',
            ...
            selectbackground=BG_MID, selectforeground=TEXT,
            arrowcolor=ACCENT, borderwidth=1)

        style.configure('Horizontal.TScale',
            background=BG_DARK, troughcolor=BG_MID,
    ...
    def _build_ui(self):
        ...
        #          bg=BG_MID, fg=ACCENT,
        #          font=('Segoe UI', 14, 'bold')).pack(side='left', padx=18, pady=10)
        tk.Label(hdr, text="🌍 QUAKEVIEW | Earthquake Analytics Dashboard",
                 bg=BG_MID, fg=ACCENT,
                 font=('Segoe UI', 14, 'bold')).pack(side='left', padx=18, pady=10)
    ...
    def _build_sidebar(self, parent):
        ...
        # ── Buttons ────────────────────────────────────────────────────────
        section("ACTIONS")
        ttk.Button(sb, text="▶  Apply Filters",
                   command=self._apply_filters).pack(fill='x', padx=14, pady=(8, 4))
        ttk.Button(sb, text="↺  Reset Filters",
                   style='Outline.TButton',
                   command=self._reset_filters).pack(fill='x', padx=14, pady=4)
        ttk.Button(sb, text="⟳  Reload Data",
                   style='Outline.TButton',
                   command=self._start_loading).pack(fill='x', padx=14, pady=4)
    ...
    # ── Filter callbacks ──────────────────────────────────────────────────────
    def _on_min_mag_change(self, val):
        self.min_mag_label.config(text=f"{float(val):.1f}")

    def _on_max_mag_change(self, val):
        self.max_mag_label.config(text=f"{float(val):.1f}")
